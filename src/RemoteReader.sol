...
address constant RSC_PRECOMPILE = address(0x0000000000000000000000000000000000000101);

// L1SLOAD precompile (addr 0x0102)
address constant L1SLOAD_PRECOMPILE = address(0x0000000000000000000000000000000000000102);
...
contract RemoteReader {
    ...
    function readCountRSC(address l1Counter) external {
        ...
        bytes memory inner = abi.encodeWithSelector(bytes4(keccak256("count()")));
    ...
    function readCountL1SLOAD(address l1Counter) external {
        ...
        (bool ok, bytes memory ret) = L1SLOAD_PRECOMPILE.staticcall(abi.encode(l1Counter, slot0));
